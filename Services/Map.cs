...
public class Map {
    ...
    private string GenerateMapUrl(double lat, double lon, int zoom, int width, int height) {
        return
            $"https://static-maps.yandex.ru/1.x/?ll={lon.ToString(CultureInfo.InvariantCulture)},{lat.ToString(CultureInfo.InvariantCulture)}&z={zoom}&size={width},{height}&l=map&apikey={_yandexApikey}";
    }
    ...
    private static Image MarkLocationOnMap(Image mapImage, double lat, double lon) {
        ...
        mapImage.Mutate(ctx => { ctx.Fill(Color.Red, new EllipsePolygon(markerX, markerY, markerSize / 2)); });
